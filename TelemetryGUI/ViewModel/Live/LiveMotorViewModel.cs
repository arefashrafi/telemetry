...
{
    public class LiveMotorViewModel : BaseViewModel
    {
        ...
        private void OnTick(object sender, EntityEventArgs e)
        {
            ...
            lock (_syncRoot)
            {
                ...
                foreach (var channel in _channelViewModels)
                {
                    ...
                    var dateTime = DateTime.ParseExact(_motor.Time, "yyyy-MM-dd HH:mm:ss.fff",
                    ...
                    {
                        double data =
                            Convert.ToDouble(_motor.GetType().GetProperty(channel.ChannelName)?.GetValue(_motor, null));
