...
namespace TelemetryGUI.Util
{
    public class EventSource
    {
        ...
        {
            if (data.GetType() == typeof(Motor))
            {
                EventMotor.Invoke(null, new EntityEventArgs(data,time));
            }
            if (data.GetType() == typeof(Bms))
            {
                EventBms.Invoke(null, new EntityEventArgs(data,time));
            }
            if (data.GetType() == typeof(Error))
            {
                EventError.Invoke(null, new EntityEventArgs(data,time));
            }
            if (data.GetType() == typeof(Gps))
            {
                EventGps.Invoke(null, new EntityEventArgs(data,time));
            }
            if (data.GetType() == typeof(MPPT))
            {
                EventGps.Invoke(null, new EntityEventArgs(data,time));
            }
