...
namespace TelemetryGUI.Util
{
    public static class SpaCalculator
    {
        ...
        private static readonly double Pi = 3.1415926535897932384626433832795028841971;
        private static readonly double SunRadius = 0.26667;

        private static readonly int L_COUNT = 6;
        ...
        {
            degrees /= 360.0;
            double limited = 360.0 * (degrees - Math.Floor(degrees));
        ...
        {
            degrees /= 180.0;
            double limited = 180.0 * (degrees - Math.Floor(degrees));
        ...
        private static double JulianDay(int year, int month, int day, int hour, int minute, double second, double dut1,
            double tz)
        {
            double dayDecimal = day + (hour - tz + (minute + (second + dut1) / 60.0) / 60.0) / 24.0;
            ...
            double julianDay = Integer(365.25 * (year + 4716.0)) + Integer(30.6001 * (month + 1)) + dayDecimal - 1524.5;
            ...
            ref double delEpsilon)
        {
            ...
            for (i = 0; i < Y_COUNT; i++)
            {
                double xyTermSum = DegToRad(XYTermSummation(i, x));
        ...
        private static double GeocentricRightAscension(double lamda, double epsilon, double beta)
        {
            ...
            return LimitDegrees(RadToDeg(Math.Atan2(Math.Sin(lamdaRad) * Math.Cos(epsilonRad) -
                                                    Math.Tan(DegToRad(beta)) * Math.Sin(epsilonRad),
                Math.Cos(lamdaRad))));
        }
            ...
            double xi, double h, double delta, ref double deltaAlpha, ref double deltaPrime)
        {
            ...
            double deltaAlphaRad = Math.Atan2(-x * Math.Sin(xiRad) * Math.Sin(hRad),
            ...
            double slope)
        {
            ...
            return RadToDeg(Math.Acos(Math.Cos(zenithRad) * Math.Cos(slopeRad) + Math.Sin(slopeRad) *
                                      Math.Sin(zenithRad) * Math.Cos(DegToRad(azimuthAstro - azmRotation))));
        }
        ...
        private static double SunHourAngleAtRiseSet(double latitude, double deltaZero, double h0Prime)
        {
            ...
            double argument = (Math.Sin(DegToRad(h0Prime)) - Math.Sin(latitudeRad) * Math.Sin(deltaZeroRad)) /
        ...
        private static void CalculateEOTAndSunRiseTransitSet(ref SPAData spa)
        {
            ...
            double h0 = SunHourAngleAtRiseSet(spa.Latitude, delta[(int) TERM4.JD_ZERO], h0Prime);
        ...
        {
            int result = ValidateInputs(ref spa);
